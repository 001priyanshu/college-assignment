...
export const Home = () => {
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/recipe/getAllRecipes");
  ...
  const toggleFavorite = async (recipeID) => {
    try {
      if (isRecipeFavorite(recipeID)) {
        ...
        await axios.delete(`http://localhost:5000/api/user/removeFavRecipe/${recipeID}`, {
          data: { userID, recipeID },
        });
        setFavoriteRecipes((prevFavorites) => prevFavorites.filter((id) => id !== recipeID));
      } else {
        // Add to favorites
        await axios.put(`http://localhost:5000/api/user/addFavRecipe/${recipeID}`, {
          recipeID,
          userID,
        });
  ...
  const deleteRecipe = async (recipeID) => {
    try {
      await axios.delete(`http://localhost:5000/api/recipe/deleteRecipe/${recipeID}`);
